...
use App\Http\Middleware\Autheticate;
...
class ThreadController extends Controller
{
    ...
    {
        return $this->middleware([Authenticate::class, EnsureEmailIsVerified::class])->expect(['index', 'show']);
    }
    ...
    public function store(ThreadStoreRequest $request)
    {
        ...
        return redirect()->route('thread.index')->with('success', 'Thread Created!');
    }
    ...
    public function unsubscribe(Request $request, category $category, Thread $thread)
    {
        ...
        return redirect()->route('thread.show', [$thread->category->slug(), $thread->slug()])
            ->with('success', 'You have been unsubscribed to this thread');
    }
}
